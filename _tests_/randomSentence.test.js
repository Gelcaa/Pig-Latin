...
describe("randomSentence", () => {
  test("should return a random sentence", async () => {
    ...
    mockRS.getRandomSentence.mockReturnValue("random sentence");

    const result = await generate();

    expect(result.statusCode).toBe(StatusCodes.OK);
    expect(result.headers["Content-Type"]).toBe("application/json");
    expect(JSON.parse(result.body).randomSentence).toBe("random sentence");
  });

  test("should return an error", async () => {
    mockRS.getRandomSentence.mockImplementation(() => {
      throw new Error("Error in generating a random sentence");
    });
